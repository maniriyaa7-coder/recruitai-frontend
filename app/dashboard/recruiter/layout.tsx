...
import { LayoutGrid, Users, BarChart3, Settings, LogOut, Menu, X, Briefcase, FileText, Video, Bell } from 'lucide-react'
...
const sidebarItems = [
  { icon: LayoutGrid, label: 'Overview', href: '/dashboard/recruiter' },
  { icon: Users, label: 'Candidates', href: '/dashboard/recruiter/candidates' },
  { icon: Briefcase, label: 'Jobs', href: '/dashboard/recruiter/jobs' },
  { icon: FileText, label: 'Resume Analysis', href: '/dashboard/recruiter/resume-analysis' },
  { icon: BarChart3, label: 'AI Analysis', href: '/dashboard/recruiter/ai-analysis' },
  { icon: Video, label: 'Interviews', href: '/dashboard/recruiter/interviews' },
  { icon: Bell, label: 'Notifications', href: '/dashboard/recruiter/notifications' },
  { icon: BarChart3, label: 'Analytics', href: '/dashboard/recruiter/analytics' },
        ...
        <div className="p-6 flex items-center justify-between">
          {sidebarOpen && (
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-foreground">Recruiter Portal</span>
            </div>
          )}
          <button
      ...
      <div className="flex-1 flex flex-col overflow-hidden md:ml-0">
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-lg font-semibold text-foreground">Recruiter Dashboard</h1>
          </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="w-10 h-10 rounded-full bg-primary/30 border border-primary/50 flex items-center justify-center cursor-pointer">
                <span className="font-semibold text-sm">
                  {user?.name?.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2) || 'R'}
                </span>
              </div>
            </div>
          </div>
